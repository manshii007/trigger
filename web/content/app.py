...
class InstagramScraper(object):
    ...
    def get_media(self, dst, executor, future_to_item, user):
        ...
        for item in tqdm.tqdm(self.query_media_gen(user), desc='Searching {0} for posts'.format(username),
            ...
            # -Filter command line
            captions.append(item['edge_media_to_caption']['edges'][0]['node']['text'])
            if self.filter:
                if 'tags' in item:
            ...
            if self.maximum != 0 and iter >= self.maximum:
                break
        print(captions)
        print("Total : {}".format(len(captions)))
    ...
    def query_media(self, id, end_cursor=''):
        params = '{{"id":"{0}","first":12,"after":"{1}"}}'.format(id, end_cursor)
        print(params)

...
import errno, random, subprocess, multiprocessing
...
def convert(file_path):
    ...
    reseed = ["TitleCode", "DescriptorCode", "BrandNameCode", "BrandSectorCode", "BrandCategoryCode", "AdvertiserCode",
              "AdvertisingGroupCode", "CommercialProgramNameCode", "PromoProgramNameCode", "PromoTypeCode", "PromoCategoryCode"]
    ...
    dt = os.path.dirname(file_path).split("/")[-1]
    wk = os.path.dirname(file_path).split("/")[-2]
    output_path = os.path.join(args.dir, 'converted', wk, dt, os.path.basename(file_path))
...
if __name__ == '__main__':
    ...
    reports = os.path.join(args.dir, 'Week*', '*', '*.xml')

    pool = multiprocessing.Pool(4)
    files = glob.glob(reports)
    for _ in tqdm.tqdm(pool.imap_unordered(convert, files)):
        pass
    pool.close()
    pool.join()
